...
def run_remote_rtp(host, config):
    ...
    try:
        session_check_command = f"{runtime_prefix} && tmux list-sessions"
        print(f"[remote-step] check-sessions: ssh {host} \"bash -lc {shlex.quote(session_check_command)}\"")
        existing_sessions = _ssh_bash_run(host, session_check_command, check=False)
        ...
        prepare_dir_command = f"mkdir -p {shlex.quote(remote_command_dir)}"
        print(f"[remote-step] prepare-dir: ssh {host} \"bash -lc {shlex.quote(prepare_dir_command)}\"")
        _ssh_bash_run(host, prepare_dir_command)
        print(f"[remote-step] upload-script: scp {local_script_path} {host}:{remote_command_file}")
        _run_checked(["scp", local_script_path, f"{host}:{remote_command_file}"])
        chmod_command = f"chmod +x {shlex.quote(remote_command_file)}"
        print(f"[remote-step] chmod-script: ssh {host} \"bash -lc {shlex.quote(chmod_command)}\"")
        _ssh_bash_run(host, chmod_command)
        install_command = f"{runtime_prefix} && pip install {shlex.quote(install_path)}"
        print(f"[remote-step] install-game: ssh {host} \"bash -lc {shlex.quote(install_command)}\"")
        _ssh_bash_run(host, install_command)
        tmux_start_command = f"{runtime_prefix} && tmux new -d -s {shlex.quote(session_name)} {shlex.quote(remote_script_runner)}"
        print(f"[remote-step] start-tmux: ssh {host} \"bash -lc {shlex.quote(tmux_start_command)}\"")
        _ssh_bash_run(
            host,
            tmux_start_command,
        )
        print(f"[remote-step] verify-tmux: ssh {host} \"bash -lc {shlex.quote(session_check_command)}\"")
        started_sessions = _ssh_bash_run(host, session_check_command, check=False)
